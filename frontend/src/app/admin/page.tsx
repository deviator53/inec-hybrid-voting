...
export default function AdminPage() {
  ...
  const fetchElectionStatus = async () => {
    try {
      ...
      if (data.success) {
        ...
        setMessage({ text: data.error || "Failed to fetch election status", type: "error" });
      }
    } catch (error: any) {
      setMessage({ text: error.message || "Failed to connect to backend", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const activateElection = async () => {
    if (!confirm("Are you sure you want to activate the election? This will allow votes to be cast.")) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessage({ text: data.error || "Failed to activate election", type: "error" });
      }
    } catch (error: any) {
      setMessage({ text: error.message || "Failed to activate election", type: "error" });
    } finally {
      setActivating(false);
    }
  };
      ...
      <div className="bg-inec-primary border-b-4 border-yellow-500 shadow-lg">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-white font-bold text-xl">INEC - NEVS Admin</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-2 bg-white rounded-xl shadow-lg border-2 border-gray-200 p-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-800">Election Status</h2>
              <button
            ...
            {loading && !electionStatus ? (
              ...
              <div className="space-y-4">
                <div className={`p-6 rounded-xl border-2 ${
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-gray-700 font-semibold text-lg">Election Status</span>
                    <div className="flex items-center gap-3">
                      <div className={`w-4 h-4 rounded-full animate-pulse ${
                        electionStatus.isActive ? "bg-green-500" : "bg-red-500"
                      }`}></div>
                      <span className={`font-bold text-xl ${
                        electionStatus.isActive ? "text-green-700" : "text-red-700"
                      }`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <p className="text-gray-600 text-sm mb-1">Registered Candidates</p>
                    <p className="text-3xl font-bold text-gray-800">
              ...
              <div className="text-center py-12">
                ...
                <p className="text-xl text-gray-600 font-medium mb-4">Unable to load election status</p>
                <button
                  onClick={fetchElectionStatus}
